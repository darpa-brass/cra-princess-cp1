        ...
        for i in range(0, len(self.constraints_object.channels)):
            ...
            for j in range(1, table_height):
                for k in range(0, table_width):
                    if math.ceil(discretized_tas[(j-1) * self.discretization_algorithm.num_discretizations].compute_communication_length(self.constraints_object.channels[i].capacity, timedelta(microseconds=min_latency.microseconds), self.constraints_object.guard_band)*factor) > k:
                        table[j][k] = table[j-1][k]
                    else:
                        sol = table[j-1][k]
                        for l in range(((j - 1) * discretization_algorithm.num_discretizations), j * discretization_algorithm.num_discretizations):
                            wk = math.ceil(discretized_tas[l].compute_communication_length(self.constraints_object.channels[i].capacity, min_latency, self.constraints_object.guard_band) * factor)
                            if wk <= k:
            ...
            self._retrieve_tas(table, len(eligible_tas), min_latency.milliseconds *
        ...
        if row == 0:
            ...
            if table[row][column] == table[row-1][column]:
                ...
                for i in range((row-1) * self.discretization_algorithm.num_discretizations, row * self.discretization_algorithm.num_discretizations):
                    wi = math.ceil(discretized_tas[i].compute_communication_length(self.constraints_object.channels[index].capacity, min_latency, self.constraints_object.guard_band) * self.factor)
                    if wi <= column:
