...
class OrientDBHelper(BrassOrientDBHelper):
    ...
    def get_tas(self, orientdb_helper):
        ...
        """
        ta_list = orientdb_helper.get_nodes_by_type('TA')
        candidate_tas = []

        if not ta_list:
            raise TAsNotFoundException('Constraints database {0} does not contain any TAs'.format(orientdb_helper.database_name), 'orientdb_queries.get_tas')

        for ta in ta_list:
            ta_obj = TA(id_ = ta.id,
                    minimum_voice_bandwidth = ta.minimum_voice_bandwidth,
                    minimum_safety_bandwidth = ta.minimum_safety_bandwidth,
