...
from cp1.utils.orientdb_queries import OrientDBHelper
from brass_api.orientdb.orientdb_helper import BrassOrientDBHelper
from cp1.common.exception_class import StoreResultInitializationError
...
class OrientDBUpdates:
    def __init__(self, orientdb_helper):
        ...
        #     raise StoreResultInitializationError('orientdb_helper must be an instance of BrassOrientDBHelper. orientdb_herlper: [{0}]'.format(orientdb_helper), '__init__')
    ...
    def delete_nodes_by_class(self, orientdb_helper, clazz):
        """
        ...
        :type clazz: str
        """
        vertices = orientdb_helper.get_nodes_by_type(clazz)
        vertex_rids = [vertex._rid for vertex in vertices]
        orientdb_helper.delete_nodes_by_rid(vertex_rids)
    ...
    #         raise ValueError('mdl_frequency must be an instance of MDLFrequency. mdl_frequency: [{0}]'.format(mdl_frequency), 'store_result.__update_frequency')
