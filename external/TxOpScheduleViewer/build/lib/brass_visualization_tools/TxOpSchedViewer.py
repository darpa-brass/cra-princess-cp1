...
class RanConfig:
    ...
    def check_guardbands(self):
        ...
        for i in range(len(txop_list) - 1):
            if int(txop_list[i+1]['start']) < (int(txop_list[i]['stop']) + (self.guard_ms * 1000)):
...
class TxOp:
    ...
    def __init__(self, freq=0, start_usec=0, stop_usec=0, timeout=0):
        ...
        self.duration_usec = int(stop_usec) - int(start_usec)
...
class RadioLink:
    ...
    def calc_max_latency(self, epoch_usec):
        ...
        if len(self.tx_sched) > 0:
            self.max_latency_usec = (int(epoch_usec) - (int(self.tx_sched[-1].stop_usec))) + \
        ...
        for i in range(len(self.tx_sched) - 1):
            temp_latency = int(self.tx_sched[i+1].start_usec) - (int(self.tx_sched[i].stop_usec))
